class Polygon{
    ...
    static createHex(x,y, diam){
        return new Polygon([new Vector([x, y-diam/2]), new Vector([x+Math.sqrt(3)/2*diam/2, y-diam/4]),
                            new Vector([x+Math.sqrt(3)/2*diam/2, y+diam/4]), new Vector([x, y+diam/2]),
                            new Vector([x-Math.sqrt(3)/2*diam/2, y+diam/4]), 
                            new Vector([x-Math.sqrt(3)/2*diam/2, y-diam/4])]);
    }

    fill(ctx, scale = 1){
        ctx.beginPath();
        ctx.moveTo(this.vertices[0].coords[0], this.vertices[0].coords[1]);
        for(let i=0; i<this.vertices.length; i++){
            ctx.lineTo(this.vertices[(i+1)%this.vertices.length].coords[0], 
                        this.vertices[(i+1)%this.vertices.length].coords[1]);
        }
        ctx.fill();
        ctx.closePath();
    }
